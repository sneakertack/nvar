...
function nvar(opts) {
  ...
  let exportPrefixEncountered = false;
    ...
    switch (mode) {
      ...
      case 'k': // Suspected key declaration.
        ...
        if (char === '\\') { // Encountered a backslash? Only backslash-newlines are allowed on the LHS.
          ...
          // Alternatively, 1 'export' prefix is allowed. Otherwise it's disallowed, flow down to the x-modeswitch.
      ...
      case 'v': // Value declaration.
        exportPrefixEncountered = false; // Reset exportPrefixEncountered (it's only applicable during the 'n' and 'k' modes).
