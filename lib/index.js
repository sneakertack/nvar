  ...
  while (source[i] !== undefined) {
    ...
    switch (mode) {
      case 'n': // Start state.
        ...
        return console.log(new Error('Unrecognized character: '+char+' (position '+i+')')); // TODO: Change to throw.
      ...
      case 'v': // Value declaration.
        ...
        while (mode === 'v' && (char = source[i])) {
          switch (quoteMode) {
            ...
            case 'd':
              if (char === '\\') {
                word += source.slice(marker, i).replace(/\$({?)([a-z_]\w*)(}?)/gi, (match, p1, p2, p3) => {
            ...
            default: // Unquoted. You might meet ', ", \, $, space/tab, or newline.
              ...
              if (char === "'") {word += source.slice(marker, i).replace(/\$({?)([a-z_]\w*)(}?)/gi, (match, p1, p2, p3) => {
                if ((p1 === '' && p3 === '') || (p1 === '{' && p3 === '}')) return target[p2] || '';
                return match;
              }); quoteMode = 's'; marker = ++i; break;}
              if (char === '"') {word += source.slice(marker, i).replace(/\$({?)([a-z_]\w*)(}?)/gi, (match, p1, p2, p3) => {
                if ((p1 === '' && p3 === '') || (p1 === '{' && p3 === '}')) return target[p2] || '';
                return match;
              }); quoteMode = 'd'; marker = ++i; break;}
              if ([' ', '\t', '\n'].indexOf(char) !== -1) {
                word += source.slice(marker, i).replace(/\$({?)([a-z_]\w*)(}?)/gi, (match, p1, p2, p3) => {
                  if ((p1 === '' && p3 === '') || (p1 === '{' && p3 === '}')) return target[p2] || '';
                  return match;
